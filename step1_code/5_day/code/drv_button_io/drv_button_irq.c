...
{
	drv_button_irq_dev->have_value = 0;
...
ssize_t drv_button_irq_read (struct file *filep, char __user *buff, size_t count, loff_t *offset)
{
	...
	if ((filep->f_flags & O_NONBLOCK) && (!drv_button_irq_dev->have_value))
	{
		return -EAGAIN;
	}
	...
	wait_event_interruptible(drv_button_irq_dev->wait_queue_head, drv_button_irq_dev->have_value);
...
irqreturn_t drv_button_irq_handler_t(int no, void *data)
{
	...
	drv_button_irq_dev->have_value = 1;
	
	// 唤醒指定的等待队列
	// 参数:等待队列头
	wake_up_interruptible(&drv_button_irq_dev->wait_queue_head);
...
static int __init drv_button_irq_init(void)
{
	...
	// 初始化等待队列头
	// 参数1:待初始化的等待队列头
	init_waitqueue_head(&drv_button_irq_dev->wait_queue_head);
